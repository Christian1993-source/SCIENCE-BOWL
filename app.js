...
const els = {
  ...
  resetBankBtn: document.getElementById('reset-bank-btn'),
...
function parseOptionLine(line) {
  ...
  const match = cleaned.match(/^([A-Da-d])\s*[\).:\-]\s*(.+)$/);
  if (!match) {
    return null;
  }
...
function splitQuestionSections(rawText) {
  const normalized = String(rawText || '').replace(/\r\n/g, '\n');
  const lines = normalized.split('\n');
  const hasQPrefixedQuestions = lines.some((line) => /^q\s*:/i.test(line.trim()));

  if (hasQPrefixedQuestions) {
    const sections = [];
    let current = [];
...
function looksLikeCommentBlock(prompt) {
  ...
  if (/(debes|puedes|todo super|responses?|respuestas?|answers?|letters?)/i.test(value) && !/[?:]$/.test(value)) {
    return true;
  }
...
function parseQuestionSection(block, blockIndex) {
  ...
  if (!prompt) {
    return { question: null, error: `Question ${blockIndex + 1}: missing prompt.` };
  }
  ...
  if (options.length) {
    ...
    if (!correctKey) {
      return { question: null, error: `Question ${blockIndex + 1}: MCQ requires * on the correct option.` };
    }
    ...
    if (!question) {
      return { question: null, error: `Question ${blockIndex + 1}: invalid MCQ format.` };
    }
  ...
  if (!answerText) {
    if (looksLikeCommentBlock(prompt)) {
      return { question: null, error: '', ignored: true };
    }
    return { question: null, error: `Question ${blockIndex + 1}: missing Answer:.` };
  }
  ...
  if (numericAnswer !== null || tolerance !== null) {
    if (numericAnswer === null) {
      return { question: null, error: `Question ${blockIndex + 1}: invalid numeric Answer.` };
    }
    ...
    if (!question) {
      return { question: null, error: `Question ${blockIndex + 1}: invalid numeric question.` };
    }
  ...
  if (!fill) {
    return { question: null, error: `Question ${blockIndex + 1}: invalid fill question.` };
  }

  return { question: fill, error: '' };
}

function parseQuestionBlocks(rawText) {
  const blocks = splitQuestionSections(rawText);
  const questions = [];
  const errors = [];

  blocks.forEach((block, blockIndex) => {
    const parsed = parseQuestionSection(block, blockIndex);
    if (parsed.ignored) {
      return;
    }
    if (parsed.error) {
...
function loadQuestionBank() {
  ...
  if (!raw) {
    setQuestionBank([], 'Question bank is empty. Paste questions and save.');
    els.bankInput.value = '';
    return;
  }
  ...
  try {
    ...
    setQuestionBank(normalized, normalized.length ? 'Question bank ready.' : 'Question bank is empty. Paste questions and save.');
    els.bankInput.value = bankToText(state.questionBank);
  } catch {
    setQuestionBank([], 'Stored bank was invalid and has been cleared.');
    els.bankInput.value = '';
  }
}
...
function handleChoicePick(choiceKey) {
  ...
  queueDecision(isCorrect ? 'correct' : 'wrong', 180, true);
}
...
function clearQuestionBank() {
  ...
  state.result.message = 'LEFT WINS!';
  state.result.subtext = '';

  setQuestionBank([], 'Question bank cleared. 0 questions loaded.');
  els.bankInput.value = '';
  setStatusLine('Question bank cleared. Paste a new set to begin.');
  render();
}
...
function showArenaFlash(player, label, durationMs = 900) {
  if (!els.buzzFlash) {
    return;
  }
  ...
  const side = player === 'right' ? 'right' : 'left';
  els.buzzFlash.textContent = `${side.toUpperCase()} ${label}`;
  els.buzzFlash.hidden = false;
  els.buzzFlash.classList.remove('show', 'left-theme', 'right-theme');
  els.buzzFlash.classList.add(`${side}-theme`);
...
function showBuzzFlash(player) {
  showArenaFlash(player, 'BUZZED!', 900);
}

function showTurnFlash(player, phase) {
  const label = phase === PHASES.steal ? 'STEAL TURN!' : 'ANSWER TURN!';
  showArenaFlash(player, label, 1100);
}
...
function enterSteal(reasonText) {
  ...
  showTurnFlash(state.turnPlayer, PHASES.steal);
  playStealCue();
...
function render() {
  ...
  const activeTurn = [PHASES.answer, PHASES.steal].includes(state.phase) ? state.turnPlayer : null;
  const resolvedHighlightPlayer =
    state.phase === PHASES.steal
      ? state.turnPlayer
      : [PHASES.answer, PHASES.roundResult, PHASES.waitingNextRound].includes(state.phase)
        ? state.buzzedPlayer
        : null;
  const dimOpponent =
    [PHASES.answer, PHASES.steal].includes(state.phase) && Boolean(activeTurn)
      ? getOppositePlayer(activeTurn)
      : [PHASES.roundResult, PHASES.waitingNextRound].includes(state.phase) && Boolean(state.buzzedPlayer)
        ? getOppositePlayer(state.buzzedPlayer)
        : null;

  els.buzzLeft.classList.toggle('winner', resolvedHighlightPlayer === 'left');
  els.buzzRight.classList.toggle('winner', resolvedHighlightPlayer === 'right');
  els.buzzLeft.classList.toggle('loser', dimOpponent === 'left');
  els.buzzRight.classList.toggle('loser', dimOpponent === 'right');
  els.buzzLeft.classList.toggle('turn-active', activeTurn === 'left');
  els.buzzRight.classList.toggle('turn-active', activeTurn === 'right');
...
function handleBankSave() {
  ...
  if (parsed.errors.length) {
    setSettingsMessage(parsed.errors[0], true);
    return;
  }

  setQuestionBank(parsed.questions, `Bank replaced with exactly ${parsed.questions.length} questions.`);
}
...
function bindEvents() {
  ...
  els.questionChoices.addEventListener('pointerdown', handleChoiceInteraction);
  els.questionChoices.addEventListener('click', handleChoiceInteraction);
  ...
  els.resetBankBtn.addEventListener('click', () => {
    if (window.confirm('Clear the full question bank and reset the game to 0 questions?')) {
      clearQuestionBank();
    }
  });
